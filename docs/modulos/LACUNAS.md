...
| **o `repo` do `nfse-municipal` aponta para repositório privado** | o `MODULO.md` está espelhado aqui e o conhecimento é consultável, mas a M1 não alcança os artefatos e a F6 não tem o que copiar. A procedência do módulo o descreve como "open-source e auto-hospedável", o que hoje não confere | tornar `bittencourtthulio/ExpxNFe` público, ou corrigir a procedência para dizer que o código é privado |
